...
public class todoDAO {
	...
	public List<Todo> findAll(){
		...
		try {
			this.connect();
			ps=db.prepareStatement("SELECT * FROM todos ORDER BY importance DESIC");
			rs=ps.executeQuery();
			while(rs.next()) {
				int id=rs.getInt("id");
	...
	public void inserOne(Todo todo) {
		try {
			this.connect();
			ps=db.prepareStatement("INSERT * INTO todos (title,importance) VALUE=?");
	...
	public Todo findOne(int id) {
		...
		try {
			this.connect();
			ps=db.prepareStatement("SELECT * FROM todos WHRER id=?");
		} catch (NamingException | SQLException e) {
			// TODO 自動生成された catch ブロック
			e.printStackTrace();
		}finally {
